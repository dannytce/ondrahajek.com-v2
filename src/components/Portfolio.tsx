...
export function Portfolio({ slug: incomingSlug, portfolios }: PortfolioProps) {
  ...
  return (
    <div className="w-full max-w-container mx-auto px-[15px] xl:px-0 m-0 p-0 list-none grid gap-5 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
      ...
      <ul className="m-0 p-0 list-none grid gap-5 w-full grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        {portfolios.map((portfolio, index) => {
          ...
          <li key={portfolio.slug ?? `portfolio-${index}`}>
            ...
            >
              ...
              <h4 className="text-secondary text-[1.5rem] font-normal -mt-[1.4em] md:-mt-[1.8em] lg:-mt-[2.5em] ml-5 relative z-10 transition-opacity group-hover:opacity-25">
                {portfolio.subtitle}
              </h4>
            </a>
          </li>
