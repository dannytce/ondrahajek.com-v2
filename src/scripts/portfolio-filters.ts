    ...
    const applyFilters = () => {
      ...
      subcategoryButtons.forEach((button) => {
        button.dataset.active =
          button.dataset.filterSubcategory === activeSubcategory
            ? 'true'
            : 'false'
      })
      ...
      items.forEach((item) => {
        ...
        const matchesSubcategory =
          !activeSubcategory || subcategories.includes(activeSubcategory)
        const matchesYear = !activeYear || year === activeYear
        const matchesLocation = !activeLocation || location === activeLocation
        const matchesQuery = !query || haystack.includes(query)

        const visible =
          matchesSubcategory && matchesYear && matchesLocation && matchesQuery
